...
namespace GenHTTP.Core
{
    ...
    internal class HttpParser
    {
        ...
        internal HttpParser(Socket socket, ClientHandler handler)
        {
            _Handler = handler;
            _Socket = socket;
            _BufferSize = handler.Server.Configuration.ReceiveBuffer;
            _Timeout = handler.Server.Configuration.Timeout;
        }
        ...
        private void Parse()
        {
            ...
            {
                // log this error (debug help)
                _Handler.Server.Log.WriteLineColored("Failed to parse request from client '" + _Handler.IP + ":" + _Handler.Port + "': " + e.Message + Environment.NewLine + Environment.NewLine + e.StackTrace, ConsoleColor.Red);
            }
        }
        ...
        {
            _RequestsPending++;
            string connection = _CurrentRequest["connection"].ToLower();
            if (_FirstConnection)
            {
