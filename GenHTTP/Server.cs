...
2009/10/29  Andreas Nägeli        Added this comment; Refactored the Server class
...
namespace GenHTTP {
  ...
  public class Server : MarshalByRefObject {
    ...
    public void Listen() {
      ...
      if (_Configuration.RemotingEnabled) {
        // check, whether the channel is already registered
        if (RemotingConfiguration.ApplicationName != "GenHTTP") {
          try {
            // some security settings to enable events via remoting
            ...
            props["port"] = _Configuration.RemotingPort;
            props["typeFilterLevel"] = TypeFilterLevel.Full;
            // create channel
            TcpChannel channel = new TcpChannel(props, clientProvider, serverProvider); 
            RemotingConfiguration.ApplicationName = "GenHTTP";
            ChannelServices.RegisterChannel(channel, false);
            RemotingConfiguration.RegisterWellKnownServiceType(typeof(InstanceProvider), "InstanceProvider", WellKnownObjectMode.Singleton);
          }
          catch (Exception e) {
            // could no bind the TCP channel
            _Log.WriteTimestamp();
            _Log.WriteLineColored("Failed to initialize remoting interface: " + e.Message, ConsoleColor.Red);
          }
        }
      }
      // start the timer
