...
namespace GenHTTP.Modules.Websites.Sites
{
    ...
    public sealed class WebsiteRenderer : IRenderer<TemplateModel>
    {
        ...
        {
            return await Theme.Renderer.RenderAsync(await GetWebsiteModel(model));
        }

        public async ValueTask RenderAsync(TemplateModel model, Stream target)
        {
            await Theme.Renderer.RenderAsync(await GetWebsiteModel(model), target);
        }
        ...
        {
            var menu = await Menu.GetMenuAsync(model.Request, model.Handler);
